...
Write-Host ">>> Tests: auth-service (TOTP / 2FA)" -ForegroundColor Yellow
& $Maven -f "$Root\pom.xml" -B -pl auth-service -am test "-Dtest=TotpServiceTest" "-Dsurefire.failIfNoSpecifiedTests=false"
